...
def assign_fragments(align_table,
                     fragment_df,
                     output_csv,
    ...
    -----------------------------
        ...
        : fragment_df (pd.DataFrame): the digested reference genome conforming to nanopore FragmentRecord
        : output_csv (stream): expected that this will be sys.stdout
    ...
    """
    align_table = pd.read_csv(align_table)
    fragment_df = pd.read_csv(fragment_df)
    ...
    pore_c_table =  pore_c_table.reset_index(drop=True).sort_values(["read_idx", "align_idx"], ascending=True)
    pore_c_table.to_csv(output_csv, index=False)
...
if __name__ == "__main__":
    assign_fragments(sys.argv[1], sys.argv[2], sys.stdout)